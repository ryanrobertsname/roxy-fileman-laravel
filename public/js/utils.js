...
RoxyUtils.LoadConfig = function(){
  $.ajax({
      url: assets_path.concat('/conf.json'),
      dataType: 'json',
      async:false,
...
RoxyUtils.LoadLang = function(){
  var langUrl = assets_path;
  if(RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == 'auto'){
    if(RoxyUtils.GetUrlParam('langCode')){
      langUrl = langUrl.concat('/lang/' + RoxyUtils.GetUrlParam('langCode').substr(0, 2).toLowerCase() + '.json');
    }
    else {
      var language = window.navigator.userLanguage || window.navigator.language;
      langUrl = langUrl.concat('/lang/' + language.substr(0, 2) + '.json');
    }
    ...
    if(RoxyFilemanConf.LANG){
      langUrl = langUrl.concat('/lang/' + RoxyFilemanConf.LANG.substr(0, 2).toLowerCase() + '.json');
      if(!RoxyUtils.FileExists(langUrl))
        langUrl = '';
      }
  }
  if(!langUrl)
    langUrl = assets_path.concat('/lang/en.json');
