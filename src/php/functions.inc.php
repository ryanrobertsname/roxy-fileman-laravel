...
function verifyAction($action){
  //this verification is not necessary for laravel port
  return;
...
$tmp = json_decode(file_get_contents(public_path().\Config::get('roxy-fileman-laravel::assets_uri').'/conf.json'), true);
if($tmp){
  foreach ($tmp as $k=>$v)
    define($k, $v);
}
